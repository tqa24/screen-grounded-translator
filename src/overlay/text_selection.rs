...
// Shared signals to manage state from hook/timer
...
unsafe extern "system" fn tag_wnd_proc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    match msg {
        WM_TIMER => {
            ...
            // 1. Check Input State (Polling LButton)
            // We use GetAsyncKeyState for immediate hardware state
            let lbutton_down = (GetAsyncKeyState(VK_LBUTTON.0 as i32) as u16 & 0x8000) != 0;
            
            // State Transition: Idle -> Selecting
            if !IS_SELECTING && lbutton_down {
                IS_SELECTING = true;
            }
            // State Transition: Selecting -> Finished (Mouse Up)
            else if IS_SELECTING && !lbutton_down {
                // User released mouse -> Process Text
                KillTimer(hwnd, 1);
                
                // Trigger Processing
                // We run this in a thread to allow the window to close properly
                let preset_idx = CURRENT_PRESET_IDX;
                std::thread::spawn(move || {
                    // Send Ctrl+C
                    crate::overlay::utils::force_focus_and_paste(HWND(0)); // Re-using existing function? No, that pastes.
                            ...
                            if !ptr.is_null() {
                                ...
                                if size > 0 {
                                    ...
                                    if let Some(end) = wide_slice.iter().position(|&c| c == 0) {
                                        clipboard_text = String::from_utf16_lossy(&wide_slice[..end]).to_string();
                                    }
                                }
                            }
            ...
            // 2. Update Position to follow cursor
            let mut pt = POINT::default();
            GetCursorPos(&mut pt);
            // Offset slightly up-left
            SetWindowPos(hwnd, HWND_TOPMOST, pt.x - 30, pt.y - 60, 0, 0, SWP_NOSIZE | SWP_NOACTIVATE);

            // 3. Animation Logic
            if IS_SELECTING {
                ANIMATION_OFFSET -= 15.0; // Fast spin reverse
            } else {
                ANIMATION_OFFSET += 5.0; // Normal idle spin
            }
            ...
            // 4. Repaint
            paint_tag_window(hwnd, 200, 40, CURRENT_ALPHA as u8, IS_SELECTING);
            LRESULT(0)
        }
        WM_CLOSE => {
...
unsafe fn paint_tag_window(hwnd: HWND, width: i32, height: i32, alpha: u8, is_selecting: bool) {
    ...
    if !p_bits.is_null() {
        ...
        // Draw similar rounded box with glow as recording UI
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                let aa_half = 0.75;
                if d < -aa_half {
                    final_alpha = 0.85; // Opaque bg for readability
    ...
    if !p_bits.is_null() {
        ...
        for px in pixels.iter_mut() {
            ...
            if max_c > a { *px = (max_c << 24) | (r << 16) | (g << 8) | b; }
        }
    }
